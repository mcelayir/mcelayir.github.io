...
a container is a software which is packaged up with all its dependencies so that
...
So how do containers fix this problem? Containers are an operating system virtualizations bundled with the resources needed to run the application. A container
...
This comparison doesn't make sense at all since they are not alternatives but complementary products. Docker is a software that you can install on your computer and start creating and running containers. On the other hand with Kubernetes you can orchestrate the containers as pods. Kubernetes, allows container provisioning, networking, load balancing, and scaling. At the end of the day, we can develop our applications with Docker and use Kubernetes to orchestrate them.
...
Docker compose helps you to run multi-container application in a single host. It saves you starting multiple containers and creating the network which allows them to communicate. Docker compose will spin up your services as containers and create the network so that they can communicate with each other as they are different hosts whereas
Kubernetes is an orchestration which can run and manage containers on multiple hosts. It handles the scheduling of containers onto nodes in a cluster and manages workloads. Kubernetes will create the illusion of running the components on a single large computer,
...
To understand these capabilities let's explore Kubernetes further
...
On the cluster part, Kubernetes provides a bunch of nodes and a control plane. The control plane provides an interface to receive configurations, a scheduler to distribute the work
between nodes and a persistent store to keep track of the state.

On the orchestration part, Kubernetes organizes everything into a useful app, ensures that things are running smoothly and responds to changes in circumstances.

### Control Plane and Masters

The state of all of the Kubernetes objects is maintained by the `Control Plane`. Control plane is also responsible for running continuous checks to manage the objects state.
These loops will detect and respond to changes in the states to ensure that the current state matches the desired state.

Control plane also provides the API server which is responsible for handling all of the communication between components. External user components like Deployments are provided over this API.

For example, when a deployment provided, the yaml configuration file is posted to the API server and translated into a new desired state. Then this new desired state is stored by the control plane and a bunch of instructions created to start the applications and schedule those instructions to cluster nodes to make sure that the current state matches the desired state.
...
Kubernetes indeed runs containerized apps. However, you cannot run a container directly on a Kubernetes cluster. Containers must always run as wrapped inside of Pods.
...
All containers that are running inside the same pod, share the same environment. This means they will use the same memory, volumes, and network.

Multiple containers inside the same pod will come in handy that if you have tightly coupled containers that need to share the same memory and resources. However keep in mind that, the best practice is to create a Pod for a container and use complementary containers if needed. 
...
The Pod concept affects the design of the components of the application. It is better to design components to be loosely coupled, stateless and has a single responsibility.

# Deployments

A `Deployment` is a Kubernetes object, which makes easy to manage a set of identical pods. Without Deployment, one has to create, delete and update the pods manually. Deployments make sure that one or more instances of your application are available to serve user requests.
...
Imagine a scenario that involves some frontends needs to access some backends. In this case, frontends need to keep track of track of the backend addresses and find the appropriate address to interact with the backend.
...
The source code alongside with the Kubernetes and docker-compose files can be found at
...
Let's discuss a scenario that we would like to create pods for these services if the database is ready because if it is not then having those services doesn't have a point. To overcome this situation we can add an init container to check the readiness of the database.
...
This init container will start before the app container started. It is going to execute `pg_isready` to check if the database is ready. When the command returns a successful response, the init container will be completed and the app container can be started.

Also, it's important to note that, we are able to use `postgres` as the hostname to access the database since we have the service defined. 
